...
public class BoardServiceTest extends IntegrationTest {
  ...
  @Before
  public void setUp() {
    user = User.builder().account("userId").password("password").name("test user").build();
  ...
  public void editBoard() {
    ...
    BoardReqDto boardReqDto = BoardReqDto.builder().name("edit board name").build();
  ...
  private Article generatedArticle(int index, Board board) {
    ...
    Article article = Article.builder().title("title" + index).contents("contents").createdIP(IP_VALID_FORMAT_VALUE)
        .lastUpdatedIp(IP_VALID_FORMAT_VALUE).user(this.user).userName(this.user.getName()).board(board).build();
    return articleRepository.save(article);
  }

  private Board generatedBoard() {
    return Board.builder().name(CREATE_BOARD_NAME).build();
  }
